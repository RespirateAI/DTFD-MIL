...
parser.add_argument("--numGroup", default=9, type=int)
...
def main():
    ...
    save_dir = os.path.join(params.log_dir, "best_model_lung_norandom.pth")
    z = vars(params).copy()
    ...
    # print_log(
    #     f"training slides: {len(SlideNames_train)}, validation slides: {len(SlideNames_val)}",
    #     log_file,
    # )
...
def test_attention_DTFD_preFeat_MultipleMean(
    ...
    # onehot_matrix = torch.eye(3)

    # # Index the matrix using predicted class indices
    # gPred_0 = onehot_matrix[gPred_0]
    # gPred_1 = onehot_matrix[gPred_1]

    # print("GT",gt_0,gt_0.shape)
    # print("GPRED",gPred_0,gPred_0.shape)
...
def train_attention_preFeature_DTFD(
    ...
    data = LungDataset(
        "/media/ravindu/SSD-PLU3/Lung_CT_Dataset/Lung-PET-CT-Dx-NBIA-Manifest-122220/DrasNew/DrasCLR/embeddings_ct/train"
    )
    dataloader = DataLoader(data, params.batch_size, collate_fn=custom_collate)
    ...
    for batch in dataloader:
        ...
        for tidx, tslide in enumerate(slide_names_batch):
            ...
            for tindex in index_chunk_list:
                ...
                tAA = attention(subFeat_tensor).squeeze(0)
                # print("Attention", tAA.shape)
                tattFeats = torch.einsum("ns,n->ns", subFeat_tensor, tAA)  ### n x fs
