...
def main():
    ...
    dimReduction = DimReduction(2048, params.mDim, numLayer_Res=params.numLayer_Res).to(
        params.device
    )
    ...
    tsave_dict = torch.load(
        os.path.join(params.log_dir, "best_model_lung_random_binary_resnet.pth")
    )
...
def test_attention_DTFD_preFeat_MultipleMean(
    ...
    data = LungDataset(
        "/media/ravindu/SSD-PLU3/Lung_CT_Dataset/Lung-PET-CT-Dx-NBIA-Manifest-122220/DrasNew/DrasCLR/embeddings_resnet/test"
    )
            ...
            for tidx, tfeat in enumerate(batch_feat):
                ...
                for jj in range(params.num_MeanInference):
                    ...
                    for tindex in index_chunk_list:
                        ...
                        attention_weights = create_dictionary(
                            attention_weights, tslideName, tindex, tAA
                        )
    ...
    np.save("./Attention/attention_weights_resnet_softmax.npy", attention_weights)
...
def create_dictionary(dic, imgName, idx_list, attention_weights):
    for idx, val in enumerate(idx_list):
        dic[imgName][val] = attention_weights[idx]
    return dic
